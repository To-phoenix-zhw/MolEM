...
for i, (pocket_fn, ligand_fn, _, rmsd_str) in enumerate(tqdm(index)):
    ...
    try:
        ...
        ligand_path = os.path.join(path, ligand_fn[:-3] + 'sdf')
        pocket_path = os.path.join(path, pocket_fn[:-3] + 'pdb')
 
        os.system("obabel " + ligand_path + " -opdbqt -O " + os.path.join(path, ligand_fn[:-3] + 'pdbqt') + " -d")
