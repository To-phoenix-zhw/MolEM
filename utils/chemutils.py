...
#     print("pos.device", pos.device)
    pos = torch.cat([pos.t(), torch.ones(n_pos).unsqueeze(0).to(pos.device)], dim=0)
    rotated_pos = torch.mm(T.to(pos.device), pos)[:3]
...
def kabsch_torch(A, B, C):
    ...
    try:
        U, S, V = torch.svd(H.cpu())  # torch.svd may have convergence issues for GPU and CPU.
    except:
        return None
    #U, S, V = torch.svd(H.cpu())  # torch.svd may have convergence issues for GPU and CPU.
    U = U.to(A.device)
...
#         print("intersection is two connected bonds")
...
def enumerate_assemble(mol, idxs, current, next):
    ...
    ground_truth_atom_nums = ground_truth.GetNumAtoms()
    for c_mol in cand_mols:
        if c_mol.GetNumAtoms() == ground_truth_atom_nums:   
            c_mol_copy = deepcopy(c_mol)   
            if len(ground_truth_copy.GetSubstructMatch(c_mol_copy)) == ground_truth_atom_nums:
                continue
        cand_mols_filtered.append(c_mol)
    if len(cand_mols_filtered) >= 1:
        cand_mols = sample(cand_mols_filtered, 1)
